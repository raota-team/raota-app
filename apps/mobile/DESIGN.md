...
- **수량은 국물 색(`broth`)으로 말한다.** 기록량 히트맵과 분포 막대는 `broth` 네 단계(`#D6A24A` → `#5E340A`)를 쓴다. 값이 클수록 진해져서 "많이 먹을수록 국물이 진해진다"로 읽힌다. **항목별 맛 평가 오각형(레이더)의 면**도 같은 이유로 국물 색이지만, 거기서는 단계가 아니라 `broth[0]`를 **35% 투명도**로 덮는다 — 면이 반투명이라 안쪽 눈금과 축이 도형 너머로 비쳐서 "어디까지 찼는지"를 눈금과 견줘 읽을 수 있다(도형 안 눈금선 6.7:1, 2pt 먹선 13.5:1). 로딩 레이더와 결과 레이더는 같은 재료를 쓴다. 라멘 종류마다 다른 색을 주는 안은 버렸다 — 국물 색이 전부 따뜻한 갈색이라 색약에서 구분되지 않는다(검증에서 ΔE 미달). **색만으로 값을 말하지 않는다**: 옆에 이름과 숫자를 함께 두고 범례에 값을 적는다.
