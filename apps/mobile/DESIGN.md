...
colors:
  ...
  yolk: "#FFC93C"
  deep: "#00422E"
  broth: ["#D6A24A", "#B87C22", "#8E5A15", "#5E340A"]
  on-dark: "#FFFFFF"
...
- **RAOTA Red (`brand`):** 주 CTA, 활성 탭, 선택된 칩, **고르는 순간의 5축 점수**, 알림 점, 취향 일치도에만 쓴다. 이미 쌓인 평균을 보여 주는 리포트의 레이더·막대·미터는 빨강이 아니다(고르는 것이 아니라 수량이다). 누름은 `brand-pressed`, 약한 배경은 `brand-weak`. 회원번호나 통계처럼 행동이 아닌 정보를 빨강으로 강조하지 않는다.
- **수량은 국물 색(`broth`)으로 말한다.** 기록량 히트맵과 분포 막대는 `broth` 네 단계(`#D6A24A` → `#5E340A`)를 쓴다. 값이 클수록 진해져서 "많이 먹을수록 국물이 진해진다"로 읽힌다. **항목별 맛 평가 오각형(레이더)의 면**도 같은 이유로 국물 색이지만, 거기서는 단계가 아니라 `broth[0]`를 **35% 투명도**로 덮어 옅은 국물 한 겹을 만든다. **레이더의 선은 바깥 2pt 먹선 오각형 하나와 그 안의 1pt 중간 눈금(0.6) 하나뿐이다** — 눈금 넷과 축 다섯을 다 겹치면 선이 데이터 도형을 이겨 버리고, 하나도 없으면 도형의 크기를 견줄 기준이 사라진다. 값은 축 이름 옆 숫자가 말하고, 중간 눈금은 "얼마나"를 눈으로 견주게 한다. 로딩 레이더와 결과 레이더는 같은 재료를 쓴다. 라멘 종류마다 다른 색을 주는 안은 버렸다 — 국물 색이 전부 따뜻한 갈색이라 색약에서 구분되지 않는다(검증에서 ΔE 미달). **색만으로 값을 말하지 않는다**: 옆에 이름과 숫자를 함께 두고 범례에 값을 적는다.
- **빨강으로 수량을 나타내지 않는다.** 기록량 히트맵이나 분포 막대처럼 "많다/적다"를 보여 주는 곳은 `ink` 농도를 쓴다. 빨강이 수량이 되면 사용자는 빨강을 볼 때마다 "누르는 곳인가, 많다는 뜻인가"를 한 번 더 판단해야 한다. 또한 **수량을 색으로만 부호화하지 않는다** — 칸마다 접근성 라벨을 주고 범례에 숫자를 적는다.
- **Ink (`ink`, `#16181A`):** 글씨, 부품의 테두리, 번지지 않는 그림자에 쓴다. 순수 검정은 쓰지 않는다.
- **Deep (`deep`, `#00422E`):** 화면 안 **큰 구획의 어두운 면**(마이 프로필, 추천 라멘집, 가게 상세 정보, 기록 완료 밴드). 라멘집 간판의 진녹색이고, 이 앱을 다른 앱과 갈라놓는 색이다. 흰 글씨 11.6:1, 노랑 스티커 7.5:1로 안전하다. **이 면 위에 빨강을 두지 않는다**(2.4:1). 면 전용이라 글씨·테두리·그림자에는 쓰지 않고, 세그먼트의 선택 칸·토스트·먹색 배지처럼 **부품**의 어두운 면은 계속 `ink`다 — 큰 구획과 작은 부품을 색으로 갈라 둔다.
...
- **노랑은 한 번에 보이는 범위에 세 개까지. 라멘 종류 태그도 이 예산에 포함된다.** 둘은 생김새가 같아서 따로 세면 상한이 없는 것과 같다. 그래서 **카드 하나가 한 대상을 보여 줄 때만 노랑 스티커**를 쓴다(오늘의 픽, 라운지 카드, 매장 상세 머리, 지도 퀵뷰, 스타일 타일). **여러 줄이 이어지는 목록에서는 종류를 굵은 먹색 글씨로 쓴다**(`RamenTypeTag`의 `inList`) — 줄마다 노랑이 반복되면 노랑은 "여기를 봐"가 아니라 줄무늬가 되고, 정작 "오늘의 픽"이 묻힌다. 예외는 **"스타일로 찾기" 타일** 하나다: 거기서는 종류 자체가 고르는 대상이라 태그가 타일의 이름이다.
...
**색마다 하는 일이 하나씩이다.** 빨강 = 행동과 선택, 노랑 = 표시, 국물 색 = 수량, 진녹 = 큰 구획의 면, 먹색 = 글씨·테두리·작은 부품의 면. **한 색이 두 가지 일을 하지 않고, 한 가지 일에 두 색을 쓰지 않는다.** 이 표 밖의 포인트 색을 새로 만들지 않고, 장식 그라디언트를 쓰지 않는다.
...
- **하단 고정 행동 바:** 기록 저장, AI 다음 단계, 완료 화면 CTA, 매장 상세의 "가고 싶어요 / 먹은 라멘 기록하기"는 하단에 고정하고 하단 inset을 더한다. 320×568급 화면에서도 스크롤 없이 보여야 한다.
  - 하단 고정 바가 inset을 직접 먹으므로, 그 화면의 루트 `SafeAreaView`에서는 `edges`에서 `"bottom"`을 뺀다. 둘 다 먹으면 흰 바 아래로 미색 바탕이 비친다.
...
**네오 브루탈리즘 라이트.** 재료는 2pt 먹선, 단색 면, 그리고 누를 수 있는 것에만 붙는 얇은 번지지 않는 그림자다. 블러, 부드러운 그림자, 그라디언트, 투명한 면은 쓰지 않는다. **예외는 차트의 데이터 도형 하나다**(맛 평가 레이더의 `broth[0]` 35%). UI의 면이 아니라 값이 그린 도형이라 그렇고, 이 예외를 다른 곳으로 넓히지 않는다.
...
- **선.** 2pt `outline`은 **누를 수 있는 것과 묶음 카드**에 두른다: 버튼, 아이콘 버튼, 사진, 입력칸, 그리고 여러 정보를 담는 카드. 칩 · 태그 · 스티커는 1.5pt. **값 하나만 보여 주는 정보 타일(매장 혜택, 통계 칸, 표의 칸)은 1pt `border`로 둔다** — 한 화면에 2pt 상자가 예닐곱 개를 넘으면 무엇이 중요한지 지정되지 않는다. 3pt 이상은 쓰지 않는다.
- **그림자는 "키(key)"라는 표시다.** 키는 눌러서 한 가지 일을 하는 독립 버튼이다. `s`(2pt): 버튼, 아이콘 버튼. `m`(3pt): 작성 버튼(FAB)과 AI 큐레이터 배너. **카드, 사진, 통계 타일, 검색창, 세그먼트, 칩, 목록 행에는 그림자를 넣지 않는다.**
- **그림자로 "선택됨"을 나타내지 않는다.** 활성 탭과 지도의 선택된 핀은 빨강 면으로만 구분한다. 그림자를 쓰면 "지금 있는 곳"이 튀어나오고 "갈 곳"이 평평해져서, 사용자가 배운 신호가 정확히 뒤집힌다.
- **누름 표현은 세 가지뿐이다.** ① 그림자가 있는 키는 그림자만큼 오른쪽 아래로 움직이고 그림자가 사라진다(식권 발매기 버튼처럼, `pressInto`). Reduce Motion에서는 이동 애니메이션 없이 즉시 바뀐다. ② 흰 면의 카드 · 목록 행 · 칩은 `canvas-soft` 배경으로 바뀐다. ③ 사진이나 먹색 면이 깔려 배경을 바꿔도 보이지 않는 것만 `pressFade`(불투명도 0.8)를 쓴다. **화면마다 다른 불투명도를 만들지 않는다.**
...
- **매장 상세:** 사진(아래 2pt 먹선, 썸네일, 뒤로가기 · 저장은 흰 사각 버튼) → 영업 상태 · 거리 · 라멘로그 수 · 평점 한 줄 → 라멘 종류 태그 → 이름 · 지점(headline) → 특징 · 주소 → 태그 → 가게 소개 카드("AI가 요약했어요" 스티커) → 바로가기 키 네 개 → 혜택(값 하나짜리 정보 타일이라 1pt 선 두 칸) → 상세 정보(먹색 카드, 줄 사이는 흰색 18% 선) → 내 라멘로그. 하단 고정 바는 흰 면 + 위쪽 2pt 선 안에 [저장됨(흰 키) · 먹은 라멘 기록하기(빨강 키)]. 비어 있는 정보는 숨기거나 빈 상태.
...
- **종합 리포트:** 정리 연출의 레이더와 결과의 레이더는 같은 재료를 쓴다(바깥 2pt 먹선 · 1pt 중간 눈금 하나 · 데이터는 `broth[0]` 35% 면 + 2pt 먹선 윤곽, 축선과 점선 없음 — 위 Colors 절 참조). 정체성 카드(제목 + 판정 근거 한 줄, 예: "누적 42그릇 중 돈코츠 14그릇 · 육수 농도 평균 3.9") → 5축 레이더(`react-native-svg`) → 축별 막대(좋고 나쁨은 차오르고, 취향의 위치는 선 위의 한 점으로 짚는다) → 종류별 분포 → 자주 간 라멘집(순위는 홈과 같은 노랑 스티커, 사진 위에 얹지 않는다) → 다음 한 그릇. 공유는 iOS 공유 시트(`Share.share`).
...
- **Don't** 블러, 부드러운 그림자, 투명한 면, 장식 그라디언트, 빨강 글로우를 쓰지 않는다. 투명한 면의 유일한 예외는 맛 평가 레이더의 데이터 도형이다.
...
**네오 브루탈리즘 라이트(2026-09-20 채택, 같은 날 앱 전체 적용 완료)**

`src/theme` 토큰, 공통 부품(`src/components/ui`), 탭 바, 기록 FAB, 그리고 `app/**` 화면 전부에 적용했다(`dev-webview`는 개발용이라 제외). 새 의존성은 없다. 배치와 문구는 그대로 두고 겉모습(바탕 · 선 · 모서리 · 그림자 · 제목 굵기 · 종류 태그)만 바꿨다. 시안은 Claude 캔버스에 있다.

남은 차이:

- **루트 웹 프로토타입(`src/`)은 아직 예전 모습이다.** 겉모습만 앱이 앞서 있고, 화면 순서 · 문구 · 숫자는 여전히 같다.
- **Android는 번지지 않는 그림자가 안 나온다.** `elevation`으로는 만들 수 없어 지금은 테두리만 보인다(같은 모양의 `ink` 면을 뒤에 한 장 더 까는 방식이 후속 과제).
- **라멘 종류 값이 두 갈래다.** 기록은 `RAMEN_TYPES`(쇼유 · 돈코츠 …), 매장 원장은 `style`("쇼유 라멘", "토리파이탄")을 쓴다. `RamenTypeTag`가 뒤의 "라멘"을 떼서 같은 글자로 보여 주지만, "토리파이탄"처럼 `RAMEN_TYPES`에 없는 값도 태그로 나온다. 종류를 정리하려면 서버 명세의 `RamenType`부터 함께 정해야 한다.
