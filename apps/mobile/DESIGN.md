...
- **수량은 국물 색(`broth`)으로 말한다.** 기록량 히트맵과 분포 막대는 `broth` 네 단계(`#D6A24A` → `#5E340A`)를 쓴다. 값이 클수록 진해져서 "많이 먹을수록 국물이 진해진다"로 읽힌다. 라멘 종류마다 다른 색을 주는 안은 버렸다 — 국물 색이 전부 따뜻한 갈색이라 색약에서 구분되지 않는다(검증에서 ΔE 미달). **색만으로 값을 말하지 않는다**: 옆에 이름과 숫자를 함께 두고 범례에 값을 적는다.
...
- **따뜻한 빈 면(`warm-empty`, `#F5DF97`):** 기록량 히트맵의 빈 칸. 국물 색과 같은 계열이라 표가 회색 격자가 아니라 한 덩어리로 읽힌다.
