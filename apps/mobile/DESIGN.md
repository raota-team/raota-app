...
- **수량은 국물 색(`broth`)으로 말한다.** 기록량 히트맵과 분포 막대는 `broth` 네 단계(`#D6A24A` → `#5E340A`)를 쓴다. 값이 클수록 진해져서 "많이 먹을수록 국물이 진해진다"로 읽힌다. **항목별 맛 평가 오각형(레이더)의 면**도 같은 이유로 국물 색이지만, 거기서는 단계가 아니라 `broth[0]`를 **35% 투명도**로 덮어 옅은 국물 한 겹을 만든다. **레이더의 선은 바깥 오각형 하나뿐이다** — 안쪽 눈금 오각형과 축을 겹치면 선이 데이터 도형을 이겨 버린다. 값은 축 이름 옆 숫자가 말한다. 로딩 레이더와 결과 레이더는 같은 재료를 쓴다. 라멘 종류마다 다른 색을 주는 안은 버렸다 — 국물 색이 전부 따뜻한 갈색이라 색약에서 구분되지 않는다(검증에서 ΔE 미달). **색만으로 값을 말하지 않는다**: 옆에 이름과 숫자를 함께 두고 범례에 값을 적는다.
...
- **종합 리포트:** 정리 연출의 레이더와 결과의 레이더는 같은 재료를 쓴다(바깥 2pt 먹선 · 안쪽 1.5pt · 데이터는 `canvas-soft` 면 + 2pt 먹선 윤곽, 점선 없음). 정체성 카드(제목 + 판정 근거 한 줄, 예: "누적 42그릇 중 돈코츠 14그릇 · 육수 농도 평균 3.9") → 5축 레이더(`react-native-svg`) → 축별 막대(좋고 나쁨은 차오르고, 취향의 위치는 선 위의 한 점으로 짚는다) → 종류별 분포 → 자주 간 라멘집(순위는 홈과 같은 노랑 스티커, 사진 위에 얹지 않는다) → 다음 한 그릇. 공유는 iOS 공유 시트(`Share.share`).
